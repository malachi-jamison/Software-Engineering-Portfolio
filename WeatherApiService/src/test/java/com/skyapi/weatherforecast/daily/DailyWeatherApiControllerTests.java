...
public class DailyWeatherApiControllerTests {
	...
	public void testGetByIPShouldReturn200Ok() throws Exception {
        ...
        System.out.println("Forecast 1 (before mock): " + forecast1.getId().getDayOfMonth() + ", " + forecast1.getId().getMonth());
        System.out.println("Forecast 2 (before mock): " + forecast2.getId().getDayOfMonth() + ", " + forecast2.getId().getMonth());
        ...
        .andExpect(jsonPath("$.location", is(expectedLocation))) // This might fail if location.toString() is not the exact JSON representation
        .andExpect(jsonPath("$.daily_forecast[0].day_of_month", is(16)))
        .andExpect(jsonPath("$.daily_forecast[0].month", is(7))) // Add this assertion
        .andExpect(jsonPath("$.daily_forecast[1].day_of_month", is(17))) // Add this assertion
        .andExpect(jsonPath("$.daily_forecast[1].month", is(7))) // Add this assertion
	...
	public void testUpdateShouldReturn400BadRequestBecauseNoData() throws Exception {
		...
		mockMvc.perform(put(requestURI).contentType(REQUEST_CONTENT_TYPE).content(requestBody))
			.andExpect(status().isBadRequest())
			.andExpect(jsonPath("$.error[0]", is("Daily forecast data cannot be empty")))
			.andDo(print());
	}	
