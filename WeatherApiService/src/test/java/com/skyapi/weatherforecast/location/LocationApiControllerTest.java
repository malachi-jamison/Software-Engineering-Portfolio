...
public class LocationApiControllerTest {
	...
	public void testAddShouldReturn201Created() throws Exception {
		...
		location.setCityName("Washington");
		location.setRegionName("Seattle");
		...
		.andExpect(jsonPath("$.city_name", is("Washington")))
	...
	public void testListShouldReturn200Ok() throws Exception {
		...
		location2.setCode("CALI_USA");
		location2.setCityName("Irvine");
		...
		location3.setCode("MD_USA");
		location3.setCityName("Baltimore City");
		location3.setRegionName("Maryland");
		location3.setCountryCode("US");
		location3.setCountryName("United States of America");
		...
		mockMvc.perform(get(END_POINT_PATH)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$[2].code", is("MD_USA")))
//			.andExpect(jsonPath("$.city_name", is("New York City")))
	...
	public void testListByPageShouldReturn200Ok() throws Exception {
		...
		mockMvc.perform(get(requestURI)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$._embedded.locations[2].code", is("MD_USA")))
	...
	@Test
	public void testListByPageShouldReturn204BadRequestInvalidPageNum() throws Exception {
		...
		mockMvc.perform(get(requestURI))
			...
			.andExpect(jsonPath("$.error[0]",containsString("must be greater than or equal to 1")))
			.andDo(print());
	}
	
	@Test
	public void testListByPageShouldReturn204BadRequestInvalidPageSize() throws Exception {
		...
		mockMvc.perform(get(requestURI))
			.andExpect(status().isBadRequest())
			.andExpect(jsonPath("$.error[0]",containsString("must be greater than or equal to 5")))
			.andDo(print());
	}
	...
	public void testListByPageShouldReturn400BadRequestInvalidSortField() throws Exception {
		...
		mockMvc.perform(get(requestURI))
			.andExpect(status().isBadRequest())
			.andExpect(jsonPath("$.error[0]",containsString("invalid sort field")))
			.andDo(print());
	}

	// Server should reject the method due to the path variable ("/ABCD")
	@Test
	...
	public void testGetShouldReturn200Ok() throws Exception {
		...
		Mockito.when(service.get(location.getCode())).thenReturn(location);
	...
	// When the service class tries to update the location, it will throw an
	// exception because its not saved in the DB
	@Test
	public void testUpdateShouldReturn404NotFound() throws Exception {
		...
		Mockito.when(service.update(Mockito.any())).thenThrow(new LocationNotFoundException(location.getCode()));
	...
	public void testValidateRequestBodyLocationCodeNotNull() throws Exception {
		...
		mockMvc.perform(post(END_POINT_PATH).contentType("application/json").content(bodyContent))
				...
				.andExpect(jsonPath("$.error[0]", is("Location code cannot be null"))).andDo(print());
	...
	public void testValidateRequestBodyLocationCodeLength() throws Exception {
		...
		location.setCode("");
		location.setCityName("New York City1");
		location.setRegionName("New York1");
		location.setCountryCode("US");
		location.setCountryName("United States of America1");
		location.setEnabled(true);
		...
		mockMvc.perform(post(END_POINT_PATH).contentType("application/json").content(bodyContent))
				.andExpect(status().isBadRequest()).andExpect(content().contentType("application/json"))
				.andExpect(jsonPath("$.error[0]", is("Location code must have 3-12 characters"))).andDo(print());

	}

	// The array of error messages are random. Get a return value from mockmvc,
	// which is MvcResult. Using assertion to find the messages in the response body
	@Test
